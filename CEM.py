...
class CEMAgent(Agent):
    ...
    def compile(self):
        """
        El modelo se le pasa al Agente sin compilar, esta funcion lo compila y
        declara la variable compiled True
        """
    ...
    def get_weights_flat(self,weights):
        """
        Mete todos los pesos de cada capa en un vector 1D y lo devuelve
        """
    ...
    def get_weights_list(self,weights_flat):
        """
        Convierte el vector de 1D de pesos en los pesos de cada capa
        """
        weights = []
        pos = 0
        for i_layer, size in enumerate(self.sizes):
    ...
    def select_action(self, state, stochastic=False):
        """
        Usa el modelo para obtener el array de probabilidades.
        stochastic: selecciona una de las opciones pesadas por su probabilidad
        No-stochastic: Selecciona la de maxima probabilidad
        """
    ...
    def update_theta(self,theta):
        """
        Actualiza el valor de theta, si no existe theta se inicializa con
        media y std
        """
    ...
    def forward(self, observation):
        """
        Toma el estado actual y obtiene una accion a tomar, esta es devuelta
        """
        # Select an action.
